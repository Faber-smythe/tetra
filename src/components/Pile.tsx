...
export default class Pile {
  ...
  constructor(coordinates: BABYLON.Vector2, mesh: BABYLON.Mesh, pileIndex: number, scene: BABYLON.Scene) {
  ...
  spawnPearl(name: string, color: "W" | "B", fastSpawn = false) {
    const coordinates = new BABYLON.Vector3(this.coordinates.x, this.pearls.length, this.coordinates.y)
    let newPearl
    if (fastSpawn) {
      const fastSpawnPosition = new BABYLON.Vector3(this.mesh.position.x, .7 + (this.pearls.length) * 0.38, this.mesh.position.z)
      newPearl = new Pearl(name, color, this.mesh, coordinates, this.scene, fastSpawnPosition)
    } else {
    ...
    this.mesh.physicsBody?.addConstraint(newPearl.mesh.physicsBody!, pileConstraint);
  ...
  pearlSleep(pearl?: Pearl) {
    ...
    pearl.mesh.position.y = .33 + ((this.pearls.length - 1) * 0.38)
  }
}
